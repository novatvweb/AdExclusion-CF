...
export const Sandbox: React.FC<SandboxProps> = ({ rules }) => {
  ...
  const renderInputField = (key: keyof TargetingData) => {
    ...
    return (
      <div key={key}>
        <label className="block text-[8px] font-black text-slate-400 uppercase mb-1 tracking-widest">{key.replace('_', ' ')}</label>
        <input
          type="text"
          ...
          className="w-full text-[10px] px-3 py-1.5 border border-slate-100 bg-slate-50 rounded-lg outline-none font-bold"
        />
      </div>
    );
  };
  ...
  return (
    <div className="bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden">
      <div className="px-8 py-5 border-b border-slate-100 bg-slate-50/50 flex items-center gap-3">
        ...
        <h2 className="font-black text-slate-800 uppercase tracking-tight text-sm">Simulator (Edge Testing)</h2>
      </div>
      <div className="p-8">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          ...
          <div className="md:col-span-2">{renderInputField('description_url')}</div>
        </div>

        <div className="mt-8 pt-6 border-t border-slate-100">
          <h3 className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-4">Rezultati analize:</h3>
          {activeMatches.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {activeMatches.map(m => (
                <div key={m.id} className="p-4 bg-slate-900 text-white rounded-2xl border border-slate-800 relative overflow-hidden">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></span>
                    <h4 className="text-[10px] font-black uppercase tracking-widest">{m.name}</h4>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {(m as any)._matchedValues?.map((val: string, i: number) => (
                      <span key={i} className="px-2 py-0.5 bg-green-500/10 border border-green-500/20 text-green-400 rounded-full text-[8px] font-black">MATCH: {val}</span>
                    ))}
                  </div>
                  <div className="mt-2">
                    <code className="text-[8px] font-mono text-indigo-400">{m.targetElementSelector}</code>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="text-center py-6 border border-dashed border-slate-100 rounded-2xl bg-slate-50/30">
              <p className="text-[9px] text-slate-300 font-black uppercase italic tracking-widest">Nema aktivnih pogodaka za ove podatke</p>
            </div>
          )}
        </div>
      </div>
